...
// Update only after reviewing every newly generated message for secret-bearing
// fields. Recovery words must never appear in an autogenerated description.
const expectedMessageDescriptionCount = 23;
...
''';
  ...
  if (taskCount != 6) {
    throw StateError(
      'Pinned Pigeon Swift async task count changed; expected 6, found $taskCount.',
    );
  }
  ...
  if (asyncReplyCount != 6) {
    throw StateError(
      'Pinned Pigeon Swift async reply count changed; expected 6, found $asyncReplyCount.',
    );
  }
  source = source.replaceAll(
    asyncReplySource,
...
void _replaceExpectedDescriptions(
  File file,
  RegExp unsafe,
  String replacement,
) {
...
void _redactKotlinEmergencyErrors(File file) {
  ...
  if (!unsafe.hasMatch(source)) {
    throw StateError(
      'Pinned Pigeon Kotlin error wrapper changed; review required.',
    );
  }
  source = source.replaceFirst(unsafe, '''  @Suppress("UNUSED_PARAMETER")
...
void _redactSwiftEmergencyErrors(File file) {
  ...
  if (!unsafe.hasMatch(source)) {
    throw StateError(
      'Pinned Pigeon Swift error wrapper changed; review required.',
    );
  }
  source = source.replaceFirst(
    unsafe,
