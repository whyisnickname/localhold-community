...
<#
.SYNOPSIS
Runs the schema-producing Argon2id calibration on one physical ADB device.

.DESCRIPTION
The device may be attached locally over USB or exposed by the reviewed Selectel
Mobile Farm ADB endpoint. The script
fails closed for emulators, uncommitted source, missing release APKs, ambiguous
ADB device selection, failed instrumentation or invalid evidence. It uses only
synthetic test inputs and does not upload vault or user data.
#>
[CmdletBinding()]
param(
    [string]$DeviceSerial = '',
...
$appApk = Join-Path $repositoryRoot 'community\spikes\argon2_calibration\emulator_app\app\build\outputs\apk\release\app-release.apk'
$testApk = Join-Path $repositoryRoot 'community\spikes\argon2_calibration\emulator_app\app\build\outputs\apk\androidTest\release\app-release-androidTest.apk'
$extractor = Join-Path $repositoryRoot 'community\spikes\argon2_calibration\tools\extract_android_result.py'
...
foreach ($requiredFile in @($appApk, $testApk, $extractor)) {
...
if ([string]::IsNullOrWhiteSpace($EvidenceDirectory)) {
    $EvidenceDirectory = Join-Path $repositoryRoot "output\stage2-android\$($revision.Substring(0, 12))\argon2"
}
$EvidenceDirectory = [System.IO.Path]::GetFullPath($EvidenceDirectory)
[System.IO.Directory]::CreateDirectory($EvidenceDirectory) | Out-Null
$logPath = Join-Path $EvidenceDirectory "argon2-$MemoryKib-$Operations.log"
$evidencePath = Join-Path $EvidenceDirectory "argon2-$MemoryKib-$Operations.json"
$metadataPath = Join-Path $EvidenceDirectory "argon2-$MemoryKib-$Operations-run.json"
...
Write-Host 'Only a synthetic password and salt are used by this diagnostic test.'
...
$testClass = 'dev.localhold.argon2.calibration.ArgonCalibrationInstrumentedTest'
$targets = "$testClass#localholdUtf8VectorsMatchWithoutNormalization,$testClass#selectedProfileProducesSchemaReadyEvidence"
$instrumentation = Invoke-SelectedAdb -Arguments @(
    'shell', 'am', 'instrument', '-w', '-r',
    '-e', 'class', $targets,
    '-e', 'memoryKib', $MemoryKib.ToString(),
    '-e', 'operations', $Operations.ToString(),
    '-e', 'buildRevision', $revision,
    'dev.localhold.argon2.emulator.test/androidx.test.runner.AndroidJUnitRunner'
)
...
& python $extractor $logPath $evidencePath
...
Write-Host "Validated Argon2 evidence: $evidencePath"
